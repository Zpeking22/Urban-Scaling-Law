...
# Data
data = pd.read_csv('Regional_travel_pattern_population.csv', encoding="GB18030")

# Distinguish regions and travel purposes
urban = data[(data['code']==1)]
transition = data[data['code']==2]
rural = data[data['code']==3]

urban_commuting = urban[urban['travel_pattern']==1]
urban_leisure = urban[urban['travel_pattern']==2]
urban_else = urban[urban['travel_pattern']==3]
transition_commuting = transition[transition['travel_pattern']==1]
transition_leisure = transition[transition['travel_pattern']==2]
transition_else = transition[transition['travel_pattern']==3]
rural_commuting = rural[rural['travel_pattern']==1]
rural_leisure = rural[rural['travel_pattern']==2]
rural_else = rural[rural['travel_pattern']==3]
...
def plot(x_true_renkou,y_true_count,y_true_sumdistance, x_fit_renkou,y_fit_count,y_fit_sumdistance,
    ...
    plt.grid(axis='y')
    plt.subplots_adjust(top=0.92, bottom=0.13)  # 调整底部空白区域的大小

    plt.xscale('log')
    plt.yscale('log')
    plt.title('(c)Rural_commuting', fontsize=20,fontweight='bold',loc='left')  # 设置中文标题
    plt.xlabel('Population size', fontsize=14, fontweight='bold')
    plt.ylabel('Travel demand', fontsize=14, fontweight='bold')
    plt.ylim(10**2,10**8)
    plt.xlim(10**5,10**7)
    #plt.xticks(np.linspace(min(x_true_renkou), max(x_true_renkou), 5))
    plt.legend(fontsize=10,loc='lower right')

    plt.savefig('Rural_Commuting')
    plt.show()


# Input
X_renkou,Y_count,X_renkou2,Y_sumdistance, = shuru(rural_commuting)## Need to change parameters!!!
...
print('total_distance:',x_break_count, y_break_count, k1_count, k2_count)
...
# Picture
plot(rural_commuting['population_district'],rural_commuting['total_frequency'],rural_commuting['total_distance'],
